...
from src.event.schemas import EventModel
...
@router_events.post("/events/create-event")
async def create_event(event: EventModel, user: User = Depends(current_user)):
    async with AsyncSession(async_engine) as session:
        ...
        if user.role_id == 1:
            ...
            return {"message": "The event has been created: "}, new_event
        else:
